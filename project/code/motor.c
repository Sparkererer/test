...
motor_param_t motor_zuoqian = MOTOR_CREATE(170, 0);//125
motor_param_t motor_youqian = MOTOR_CREATE(170, 0);
motor_param_t motor_zuohou = MOTOR_CREATE(170, 0);
motor_param_t motor_youhou = MOTOR_CREATE(170, 0);
...
/****************************************************************************************************************************************************/
	...
	{
		if (yuanhuan_status == 3) pid_result = Position_Pid_solve(&Position_Pid_Circle, func_limit(p_error, P_ERROR_LIMIT));
		else pid_result = Position_Pid_solve(&Position_Pid_Corner, func_limit(p_error, P_ERROR_LIMIT));
		motor_zuoqian.tar_speed = motor_zuoqian.set_speed - pid_result * 0;
		motor_youqian.tar_speed = motor_youqian.set_speed + pid_result;
		motor_zuohou.tar_speed = motor_zuohou.set_speed - pid_result * 0.5 * 0;
		motor_youhou.tar_speed = motor_youhou.set_speed + pid_result * 0.5;
	}
	if (p_error >= 10) //左转
	{
		if (yuanhuan_status == 3) pid_result = Position_Pid_solve(&Position_Pid_Circle, func_limit(p_error, P_ERROR_LIMIT));
		else pid_result = Position_Pid_solve(&Position_Pid_Corner, func_limit(p_error, P_ERROR_LIMIT));
		motor_zuoqian.tar_speed = motor_zuoqian.set_speed - pid_result;
		motor_youqian.tar_speed = motor_youqian.set_speed + pid_result * 0;
		motor_zuohou.tar_speed = motor_zuohou.set_speed - pid_result * 0.5;
		motor_youhou.tar_speed = motor_youhou.set_speed + pid_result * 0.5 * 0;
	}
	if (p_error > -10 && p_error < 10) //直线pid参数
	{
		pid_result =  Position_Pid_solve(&Position_Pid_Stright, func_limit(p_error, P_ERROR_LIMIT));
		motor_zuoqian.tar_speed = motor_zuoqian.set_speed - pid_result;
		motor_youqian.tar_speed = motor_youqian.set_speed + pid_result;
		motor_zuohou.tar_speed = motor_zuohou.set_speed - pid_result * 0.5;
		motor_youhou.tar_speed = motor_youhou.set_speed + pid_result * 0.5;
